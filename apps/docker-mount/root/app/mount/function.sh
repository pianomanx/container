...
rclone rc options/set --rc-user=${RC_USER} --rc-pass=${RC_PASSWORD} --rc-addr=localhost:${RC_ADDRESS} --json '{"vfs": {"CacheMode": 3}, "mount": {"AllowOther": true}}'
rclone rc mount/mount --rc-user=${RC_USER} --rc-pass=${RC_PASSWORD} --config=${CONFIG} --cache-dir=${TMPRCLONE} fs=union: mountPoint="${REMOTE}" mountType=mount 
